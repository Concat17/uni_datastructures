...
describe("MyStack tests", () => {
  const st1 = new MyStack<number>(5)
    .push(1)
    .push(2)
    .push(3)
    .push(4);

  const st2 = new MyStack<string>(2).push("Hello").push("world");
  const st3 = new MyStack<number>(10);
  ...
  it("pop test", () => {
    test(4, st1.clone().pop());
    const st = st1.clone();
    st.pop();
    test("1 2 3", st.toString());
    expect(() => st3.clone().pop()).to.throw("Stack is empty");
  });

  it("top test", () => {
    test(4, st1.clone().top());
    const st = st1.clone();
    st.top();
    test("1 2 3 4", st1.toString());
  ...
  it("parenthesesChecker test", () => {
    test(true, parenthesesChecker("()"));
    test(true, parenthesesChecker("{}"));
    test(true, parenthesesChecker("[{}]"));
    test(true, parenthesesChecker("(){}[]"));
    test(true, parenthesesChecker("[[]]"));
    test(true, parenthesesChecker("[()[]()][]"));
    test(false, parenthesesChecker("]"));
    test(false, parenthesesChecker(")("));
    test(false, parenthesesChecker("[]{)"));
    test(false, parenthesesChecker("(]"));
    test(false, parenthesesChecker("([)]"));
